...
function HomePage(){
  ...
  const [brief,setBrief]=useState(""); const [briefLoading,setBriefLoading]=useState(false); const [briefDone,setBriefDone]=useState(false);
  ...
  const filtered=newsFilter==="All"?NEWS:NEWS.filter(n=>n.commodity===newsFilter);
  const sectors=["Grains","Oilseeds","Livestock"];
  return (
    <div>
      {/* Morning Brief */}
      <Card style={{padding:20,borderLeft:`4px solid ${C.wheat}`,marginBottom:22}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:briefDone||briefLoading?14:0}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:20}}>☀️</span>
            <div>
              <div style={{color:C.charcoal,fontSize:14,fontWeight:700}}>Morning Market Brief</div>
              <div style={{color:C.wheatDark,fontSize:10}}>{new Date().toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</div>
            </div>
          </div>
          <button onClick={genBrief} disabled={briefLoading} style={{background:briefLoading?C.lightGrey:C.eucalyptus,color:C.white,border:"none",borderRadius:3,padding:"8px 16px",cursor:briefLoading?"not-allowed":"pointer",fontSize:11,fontFamily:"'DM Mono',monospace",fontWeight:600,flexShrink:0}}>
      ...
      {/* Prices by sector */}
      <SectionHead label="FUTURES PRICES — CME GROUP" sub="Simulated · Barchart API hook ready for live prices"/>
      {sectors.map(sector=>(
        <div key={sector} style={{marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:`repeat(${COMMODITIES.filter(c=>c.sector===sector).length},1fr)`,gap:10}}>
            {COMMODITIES.filter(c=>c.sector===sector).map(c=>{
              ...
              return (
                <Card key={c.symbol} style={{padding:14,borderTop:`3px solid ${c.color}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:16}}>{c.emoji}</span><div><div style={{color:C.charcoal,fontSize:12,fontWeight:700}}>{c.name}</div><div style={{color:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace"}}>{c.symbol}</div></div></div>
                    <SignalPill signal={sig.overall}/>
                  </div>
                  <div style={{color:C.navy,fontSize:22,fontFamily:"'DM Mono',monospace",fontWeight:700,marginBottom:1}}>{p.price.toFixed(2)}</div>
                  <div style={{color:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace",marginBottom:8}}>{c.unit}</div>
      ...
      <div style={{marginBottom:22}}>
        <SectionHead label="COMMODITY SCORECARD" sub="Multi-variable signal model"/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:8}}>
          {COMMODITIES.map(c=>{
            const s=SIGNALS[c.symbol]; const sc=s.score>=65?C.eucalyptus:s.score>=45?C.warning:C.negative;
            return (
              <Card key={c.symbol} style={{padding:12,textAlign:"center"}}>
                <div style={{fontSize:20,marginBottom:6}}>{c.emoji}</div>
                <div style={{color:C.charcoal,fontSize:11,fontWeight:600,marginBottom:6}}>{c.name}</div>
                <SignalPill signal={s.overall}/>
                <div style={{marginTop:8,height:4,background:C.lightGrey,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${s.score}%`,background:sc,borderRadius:2}}/></div>
                <div style={{color:sc,fontSize:12,fontFamily:"'DM Mono',monospace",fontWeight:700,marginTop:4}}>{s.score}</div>
              </Card>
            );
          })}
        </div>
      ...
      {/* Main grid */}
      <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 0.8fr",gap:20,marginBottom:22}}>
        {/* News */}
        <div>
          <SectionHead label="MARKET NEWS" sub="Sources linked · Click headline to read full story" action={
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {["All",...COMMODITIES.map(c=>c.symbol)].map(f=>{const co=COMMODITIES.find(x=>x.symbol===f); return <button key={f} onClick={()=>setNewsFilter(f)} style={{background:newsFilter===f?C.charcoal:C.white,color:newsFilter===f?C.white:C.charcoal,border:`1px solid ${newsFilter===f?C.charcoal:C.lightGrey}`,borderRadius:2,padding:"3px 7px",cursor:"pointer",fontSize:9,fontFamily:"'DM Mono',monospace",fontWeight:500}}>{co?`${co.emoji} ${f}`:"ALL"}</button>;})}
            </div>
          }/>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {filtered.map(n=>(
              <Card key={n.id} style={{borderLeft:`4px solid ${n.bullish?C.eucalyptus:C.negative}`,padding:"10px 14px",transition:"box-shadow 0.15s",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 12px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    {n.hot&&<span style={{color:C.warning,fontSize:9,fontFamily:"'DM Mono',monospace",fontWeight:700}}>🔥</span>}
            ...
            <div style={{padding:"8px 12px",background:C.offwhite,border:`1px dashed ${C.lightGrey}`,borderRadius:3}}><span style={{color:C.wheatDark,fontSize:10,fontFamily:"'DM Mono',monospace"}}>⚡ HOOK: Add NewsAPI key for live Ag headlines from Reuters, Bloomberg, Farm Futures, USDA.</span></div>
          </div>
        </div>
        {/* Weather + Currencies */}
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          <div>
            <SectionHead label="GLOBAL AG WEATHER" sub="Key growing regions"/>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {WEATHER_DATA.map((w,i)=>{
                const col=w.severity==="severe"?C.negative:w.severity==="moderate"?C.warning:C.eucalyptus;
                const bull=w.impact.toLowerCase().startsWith("bull");
                return (
                  <Card key={i} style={{padding:"9px 12px",borderLeft:`3px solid ${col}`,display:"flex",alignItems:"center",gap:10}}>
                    <span style={{fontSize:16,flexShrink:0}}>{w.icon}</span>
                    <div style={{flex:1,minWidth:0}}><div style={{color:C.charcoal,fontSize:11,fontWeight:600}}>{w.region}</div><div style={{color:C.charcoal,fontSize:10,lineHeight:1.3}}>{w.detail}</div></div>
                    <div style={{textAlign:"right",flexShrink:0}}><div style={{color:col,fontSize:9,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{w.severity.toUpperCase()}</div><div style={{color:bull?C.eucalyptus:w.impact==="Neutral"?C.wheatDark:C.negative,fontSize:9,fontFamily:"'DM Mono',monospace"}}>{w.impact}</div></div>
                  </Card>
                );
              })}
              <Card style={{padding:"8px 12px",background:C.navyPale,border:`1px solid ${C.navy}22`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><div style={{color:C.navy,fontSize:10,fontFamily:"'DM Mono',monospace",fontWeight:600}}>ENSO: La Niña Watch · 60% by Jun–Aug</div><div style={{color:C.charcoal,fontSize:10,marginTop:1}}>ONI −0.4°C · Bearish Brazil, Bullish US Plains</div></div>
                  <a href="https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/enso_advisory/" target="_blank" rel="noopener noreferrer" style={{color:C.wheat,fontSize:10,fontFamily:"'DM Mono',monospace",textDecoration:"none",flexShrink:0}}>NOAA →</a>
                </div>
              </Card>
            </div>
          </div>
          <div>
            <SectionHead label="CURRENCIES" sub="FRED API hook ready"/>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          <div>
            <SectionHead label="WASDE — STOCKS" sub="Mar 2026" action={<a href="https://www.usda.gov/oce/commodity/wasde/" target="_blank" rel="noopener noreferrer" style={{color:C.wheat,fontSize:9,fontFamily:"'DM Mono',monospace",textDecoration:"none"}}>USDA →</a>}/>
            <Card style={{padding:12}}>
              {Object.entries(WASDE_DATA).map(([k,d])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${C.lightGrey}`}}>
                  <span style={{color:C.charcoal,fontSize:12,textTransform:"capitalize"}}>{k}</span>
                  <span style={{color:C.navy,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{d.global_stocks} MMT</span>
                  <span style={{color:d.trend==="down"?C.eucalyptus:C.negative,fontSize:11,fontFamily:"'DM Mono',monospace",fontWeight:600}}>{d.stur}% {d.trend==="down"?"▼":"▲"}</span>
                </div>
              ))}
              <div style={{color:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace",marginTop:6}}>Next WASDE: Apr 8</div>
            </Card>
          </div>
          <div>
            <SectionHead label="COT — FUNDS" sub="Managed money" action={<a href="https://www.cftc.gov/MarketReports/CommitmentsofTraders/index.htm" target="_blank" rel="noopener noreferrer" style={{color:C.wheat,fontSize:9,fontFamily:"'DM Mono',monospace",textDecoration:"none"}}>CFTC →</a>}/>
            <Card style={{padding:12}}>
              {[{name:"Corn",net:42340,pct:62,extreme:false},{name:"Wheat",net:-18920,pct:28,extreme:false},{name:"Soybeans",net:87210,pct:74,extreme:false},{name:"Cattle",net:112400,pct:88,extreme:true}].map(c=>(
                <div key={c.name} style={{marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                    <div style={{display:"flex",alignItems:"center",gap:5}}><span style={{color:C.charcoal,fontSize:11}}>{c.name}</span>{c.extreme&&<span style={{color:C.warning,fontSize:8,fontFamily:"'DM Mono',monospace",fontWeight:700}}>⚠</span>}</div>
                    <span style={{color:c.net>0?C.eucalyptus:C.negative,fontSize:10,fontFamily:"'DM Mono',monospace",fontWeight:600}}>{c.net>0?"+":""}{c.net.toLocaleString()}</span>
                  </div>
                  <div style={{height:4,background:C.lightGrey,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${c.pct}%`,background:c.net>0?C.eucalyptus:C.negative,borderRadius:2}}/></div>
                </div>
              ))}
              <div style={{color:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace",marginTop:4}}>Mar 28 · Next: Apr 4</div>
            </Card>
          </div>
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {REPORT_CALENDAR.slice(0,4).map((r,i)=>(
                <Card key={i} style={{padding:"9px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><a href={r.url} target="_blank" rel="noopener noreferrer" style={{color:C.charcoal,fontSize:11,fontWeight:600,textDecoration:"none"}} onMouseEnter={e=>e.target.style.color=C.eucalyptus} onMouseLeave={e=>e.target.style.color=C.charcoal}>{r.name}</a><div style={{display:"flex",alignItems:"center",gap:6,marginTop:2}}><ImpBadge level={r.importance}/><span style={{color:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace"}}>{r.date}</span></div></div>
                  <span style={{color:i===0?C.eucalyptus:C.wheatDark,fontSize:9,fontFamily:"'DM Mono',monospace",fontWeight:i===0?700:400,flexShrink:0}}>{r.countdown}</span>
                </Card>
              ))}
            </div>
          </div>
      ...
      {/* Ag Equities strip */}
      <SectionHead label="AG EQUITIES WATCHLIST" sub="Simulated · Polygon.io hook ready"/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:8,marginBottom:8}}>
        {AG_EQUITIES.map(e=>{const up=e.pct>=0; return (
          <Card key={e.ticker} style={{padding:12}}>
            <div style={{color:C.navy,fontSize:12,fontFamily:"'DM Mono',monospace",fontWeight:700,marginBottom:1}}>{e.ticker}</div>
            <div style={{color:C.wheatDark,fontSize:9,marginBottom:6}}>{e.sector}</div>
            <div style={{color:C.charcoal,fontSize:14,fontFamily:"'DM Mono',monospace",fontWeight:700,marginBottom:2}}>${e.price}</div>
            <div style={{color:up?C.eucalyptus:C.negative,fontSize:11,fontFamily:"'DM Mono',monospace",fontWeight:600}}>{up?"+":""}{e.pct.toFixed(2)}%</div>
          </Card>
        );})}
      </div>
      <div style={{padding:"6px 10px",background:C.offwhite,border:`1px dashed ${C.lightGrey}`,borderRadius:3}}>
        <span style={{color:C.wheatDark,fontSize:10,fontFamily:"'DM Mono',monospace"}}>⚡ HOOK: Add Polygon.io or Alpha Vantage key for live equity prices.</span>
      </div>
    </div>
  );
}
